...
mu2=[["00","02","04","06","08","0a","0c","0e","10","12","14","16","18","1a","1c","1e"],
...
invmultd=[["00","0d","1a","17","34","39","2e","23","68","65","72","7f","5c","51","46","4b"],
...
invmulb=[["00","0b","16","1d","2c","27","3a","31","58","53","4e","45","74","7f","62","69"],
...
invmulte=[["00","0e","1c","12","38","36","24","2a","70","7e","6c","62","48","46","54","5a"],
...
invmul9=[["00","09","12","1b","24","2d","36","3f","48","41","5a","53","6c","65","7e","77"],
...
rr=""
arr = [[0]*4,[0]*4,[0]*4,[0]*4]
arr2 = [[0]*4,[0]*4,[0]*4,[0]*4]
key =[[0]*4,[0]*4,[0]*4,[0]*4]
key_de=[[0]*4,[0]*4,[0]*4,[0]*4]
plaintext=""
...
keyd=""
while (True):
    key1 = str(input("Please Enter your key "))
    keyd=key1
    if (len(key1) == 32):
        break
    print("Error")
...
w2 = np.array(key).transpose()
w3=[]
w3.append(np.array(key).transpose())
print(w3)
...
def g(W, RC):
    ...
    for i in range(0, 4):
        ...
        x1 = int(x[0], 16)
        x2 = int(x[1], 16)
        Word3[i] = ByteSub_S_Box[x1][x2]
    Word3[0] = format(change_to_be_hex(RC) ^ change_to_be_hex(Word3[0]), 'x')
    return Word3


def Key_scheduler(PW):
    New_word = [[0] * 4, [0] * 4, [0] * 4, [0] * 4]
    for i in range(0, 4):
        New_word[0][i] = format(change_to_be_hex(PW[0][i]) ^ change_to_be_hex(gWord[i]), 'x')
        # New_word2[i][0]= format(change_to_be_hex(PW[i][0]) ^ change_to_be_hex(gWord[i]), 'x')
    for i in range(0, 4):
        New_word[1][i] = format(change_to_be_hex(New_word[0][i]) ^ change_to_be_hex(PW[1][i]), 'x')

    for i in range(0, 4):
        New_word[2][i] = format(change_to_be_hex(New_word[1][i]) ^ change_to_be_hex(PW[2][i]), 'x')

    for i in range(0, 4):
        New_word[3][i] = format(change_to_be_hex(New_word[2][i]) ^ change_to_be_hex(PW[3][i]), 'x')
...
if len(plaintext) > 16:
    final_plain_txt = [plaintext[i:i + 16] for i in range(0, len(plaintext), 16)]
    zzz = len(final_plain_txt) - 1
    #print(zzz)
    while (len(final_plain_txt[zzz]) < 16):
        final_plain_txt[zzz] = final_plain_txt[zzz] + "*"

    print(final_plain_txt)
    print(zzz)
  #
    ...
    while (len(plaintext) < 16):
        plaintext = plaintext + "*"
    final_plain_txt.append(plaintext)

for i in range(0,len(final_plain_txt)):
    print("hola")
    itre=0
    last=final_plain_txt[i]
    for i in range(0, 4):
        for j in range(0, 4):
            arr[j][i] = format(ord(last[itre]), "x")
            itre += 1
    print(arr)
    print(w2)
    for i in range(0, 4):
        for j in range(0, 4):
            arr[i][j] = format(change_to_be_hex(arr[i][j]) ^ change_to_be_hex(w2[i][j]), 'x')

    # print("After ByteSub: ", arr)
    ...
    for y in range(0, 10):
        ...
        if (y != 9):
            for i in range(0, 4):
                ...
                arr[0][i] = hex(int(mu2[x1][x2], 16) ^ int(mu3[xx1][xx2], 16) ^ int(y3, 16) ^ int(y4, 16))
                arr[1][i] = hex(int(mu2[xx1][xx2], 16) ^ int(mu3[xxx1][xxx2], 16) ^ int(y1, 16) ^ int(y4, 16))
                arr[2][i] = hex(int(mu2[xxx1][xxx2], 16) ^ int(mu3[xxxx1][xxxx2], 16) ^ int(y1, 16) ^ int(y2, 16))
                arr[3][i] = hex(int(mu2[xxxx1][xxxx2], 16) ^ int(mu3[x1][x2], 16) ^ int(y2, 16) ^ int(y3, 16))
        # print(arr)

        gWord = g(key, RC[y])
        arr1 = (np.array(Key_scheduler(key)).transpose())
        arr3.append(arr1)
        key = Key_scheduler(key)

        for i in range(0, 4):
            for j in range(0, 4):
                arr[i][j] = format(change_to_be_hex(str(arr1[i][j])) ^ change_to_be_hex(arr[i][j]), 'x')
    ...
    for ww in range(9, -1, -1):
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                ...
                arr[i][j] = format(change_to_be_hex(arr[i][j]) ^ change_to_be_hex(w22[i][j]), 'x')
                if (len(arr[i][j]) == 1):
                    arr[i][j] = "0" + arr[i][j]
        ...
        if (ww != 9):
            ...
            for i in range(0, 4):
                ...
                arr[0][i] = format(
                    int(invmulte[x1][x2], 16) ^ int(invmulb[xx1][xx2], 16) ^ int(invmultd[xxx1][xxx2], 16) ^ int(
                        invmul9[xxxx1][xxxx2], 16), 'x')
                arr[1][i] = format(
                    int(invmul9[x1][x2], 16) ^ int(invmulte[xx1][xx2], 16) ^ int(invmulb[xxx1][xxx2], 16) ^ int(
                        invmultd[xxxx1][xxxx2], 16), 'x')
                arr[2][i] = format(
                    int(invmultd[x1][x2], 16) ^ int(invmul9[xx1][xx2], 16) ^ int(invmulte[xxx1][xxx2], 16) ^ int(
                        invmulb[xxxx1][xxxx2], 16), 'x')
                arr[3][i] = format(
                    int(invmulb[x1][x2], 16) ^ int(invmultd[xx1][xx2], 16) ^ int(invmul9[xxx1][xxx2], 16) ^ int(
                        invmulte[xxxx1][xxxx2], 16), 'x')


        for i in range(0, 4):
            arr[i] = arr[i][4 - i:] + arr[i][:4 - i]
    ...
    for i in range(0, 4):
        for j in range(0, 4):
            print(arr[i][j])
            print(key_de[i][j])
            arr[i][j] = format(change_to_be_hex(arr[i][j]) ^ change_to_be_hex(str(key_de[i][j])), 'x')
